...
func TestErrorCheckDetectsRubyStackTrace(t *testing.T) {
	...
	if len(findings) == 0 {
		t.Error("expected error-disclosure finding for Ruby stack trace, got 0")
	}
	if len(findings) > 0 && findings[0].CheckID != "web.error.stack-trace" {
...
func TestErrorCheckIgnoresClean500(t *testing.T) {
	...
	if len(findings) != 0 {
		t.Errorf("expected 0 findings for clean 500, got %d", len(findings))
	}
}
